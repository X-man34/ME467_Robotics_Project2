...
triad_filter = TriadEstimator(dT=time_step, kI=.3, kp=1, ka_nominal=.8, km_nominal=.2, init_conditions=(raw_accel_vector, raw_mag_vector))
...
with mujoco.viewer.launch_passive(model, mujoco_model_data) as viewer:
    ...
    for index, row in csv_data.iterrows():
        ...
        current_orientation_quat = triad_filter.time_step(raw_mag_vector, raw_gyro_vector, raw_accel_vector)
        # Save the results. 
        ...
        error_estimates.append(triad_filter.estimated_error)
        ...
        mujoco_model_data.qpos[qpos_addr_v_m_hat:qpos_addr_v_m_hat+4] = get_quat_from_vec(triad_filter.v_hat_m, negate_z=False)
        mujoco_model_data.qpos[qpos_addr_v_a_hat:qpos_addr_v_a_hat+4] = get_quat_from_vec(triad_filter.v_hat_m, negate_z=True)
